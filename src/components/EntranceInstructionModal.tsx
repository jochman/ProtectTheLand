  ...
  return <div className="absolute inset-0 z-50 grid place-items-center bg-black/75 p-4">
    <section role="dialog" aria-modal="true" aria-labelledby="intro-title" className="modal-panel w-full max-w-sm rounded-3xl bg-amber-50 p-5 text-slate-900 shadow-2xl">
      ...
      <ol className="mt-4 list-decimal space-y-2 ps-5 text-sm">
        ...
        <li>{he ? 'בחר מאחז וגזרת מקור. בדוק את התחזית ואשר פריסה: 25₪.' : 'Choose an outpost and source sector. Preview and confirm deployment: ₪25.'}</li>
      ...
      <button className="mt-2 min-h-11 w-full text-sm underline" onClick={() => { dispatch({ type: 'CLOSE_INTRO_MODAL' }); dispatch({ type: 'OPEN_TOOLKIT' }); }}>{he ? 'בחר תרחיש ואתגר' : 'Choose a scenario and challenge'}</button>
    </section>
  </div>;
}
