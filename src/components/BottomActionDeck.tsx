...
import { RULES, objective, availableTroops, totalSettlementSites } from '../game/rules';
...
export function BottomActionDeck({ state, dispatch }: { state: GameState; dispatch: Dispatch<GameAction> }) {
  ...
  const instruction = state.tutorialStep === 'observe' && state.activeBreaches.length ? (translate(state.locale, 'components.BottomActionDeck.16', []))
    ...
    : threat ? (translate(state.locale, 'components.BottomActionDeck.directThreat'))
    : state.tutorialStep === 'build' ? (construction ? translate(state.locale, 'actions.constructing', [Math.min(state.settlementsCount + 1, totalSettlementSites), totalSettlementSites, construction.progress]) : translate(state.locale, 'components.BottomActionDeck.19'))
  ...
  const progress = translate(state.locale, 'progress.counts', [state.settlementsCount, totalSettlementSites, staffed, state.settlementsCount, RULES.checkpoints - state.activeBreaches.length, RULES.checkpoints]);
  return <div className="action-deck relative z-20 flex shrink-0 flex-col gap-1 px-3 pb-2 pt-1">
    <div role="status" className="flex min-h-10 items-center rounded-xl border border-amber-300 bg-amber-50 px-2 text-start text-xs font-bold leading-snug text-slate-900">
      <div className="min-w-0 flex-1">
        {state.tutorialStep === 'done' ? <><span data-testid="conquest-progress" className="block text-[10px] tabular-nums">{progress}</span><span className="block truncate text-[10px]">{instruction}</span></> : instruction}
      </div>
    </div>
    <div className="grid grid-cols-3 gap-1.5">
      ...
      <button aria-label={translate(state.locale, 'actions.buildAria', [Math.min(state.settlementsCount, totalSettlementSites), totalSettlementSites, RULES.buildCost, RULES.settlementGrant])} className="clay-btn min-h-14 px-1 py-1 text-xs" disabled={!playing || state.budget < RULES.buildCost || !!construction || state.settlementsCount >= totalSettlementSites} onClick={() => dispatch({ type: 'BUILD_SETTLEMENT' })}>
        <span className="whitespace-nowrap">{translate(state.locale, 'actions.buildShort', [Math.min(state.settlementsCount, totalSettlementSites), totalSettlementSites])}</span><span className="whitespace-nowrap text-[10px]">{translate(state.locale, 'actions.buildReward', [RULES.buildCost, RULES.settlementGrant])}</span>
      </button>
    </div>
