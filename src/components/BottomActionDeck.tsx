...
export function BottomActionDeck({ state, dispatch }: { state: GameState; dispatch: Dispatch<GameAction> }) {
  ...
  const instruction = state.isDeployMode ? (translate(state.locale, 'components.BottomActionDeck.14', []))
    ...
    : state.tutorialStep === 'build' ? (constructing ? (translate(state.locale, 'components.BottomActionDeck.19', [])) : (translate(state.locale, 'components.BottomActionDeck.19', [])))
  ...
  return <div className="action-deck relative z-20 flex shrink-0 flex-col gap-1 px-3 pb-2 pt-1">
    <div className="flex min-h-10 items-center gap-1 rounded-xl border border-amber-300 bg-amber-50 px-2 text-slate-900">
      <button className="min-h-10 flex-1 text-start text-xs font-bold leading-snug" onClick={() => dispatch({ type: 'SHOW_INFO_POPOVER', title: translate(state.locale, 'components.BottomActionDeck.27', []), text: details })}>{instruction} <span className="text-amber-700">ⓘ</span></button>
    ...
    <div className="grid grid-cols-3 gap-1.5">
      <button className="clay-btn min-h-14 px-1 py-1 text-xs" disabled={!playing || state.reservesBatchesLeft === 0} onClick={() => dispatch({ type: 'CALL_RESERVES' })}>
        <span>{translate(state.locale, 'components.BottomActionDeck.32', [])}</span><span className="text-[11px]">+4 · {state.reservesBatchesLeft} {translate(state.locale, 'components.BottomActionDeck.32', [])}</span>
      </button>
      <button className={`clay-btn min-h-14 px-1 py-1 text-xs ${state.isDeployMode || reinforce ? 'ring-2 ring-amber-700' : ''}`} disabled={!playing || (!reinforce && !state.isDeployMode && (!exposed || !hasTroops || state.budget < RULES.deployCost))} onClick={() => dispatch(reinforce ? { type: 'SELECT_THREAT', id: threat.id } : { type: 'DEPLOY_TROOPS' })}>
        <span>{reinforce ? (translate(state.locale, 'components.BottomActionDeck.35', [])) : (translate(state.locale, 'components.BottomActionDeck.35', []))}</span><span className="text-[11px]">{reinforce ? (translate(state.locale, 'components.BottomActionDeck.35', [])) : `₪${RULES.deployCost} / ${translate(state.locale, 'components.BottomActionDeck.35', [])}`}</span>
      </button>
